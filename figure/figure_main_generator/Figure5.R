...
df = data_5[data_5$filtering == "Drosophila_melanogaster_abundant_sv",]
{
  ...
  # 3963396 SNPs
  p5A  = ggplot(df,aes(x=pos,y=mean_polymorphism,fill=color_group)) + geom_col(width=0.1,col="black") + theme_bw()+
...

{
  df$pos=c(3.73,3.13,4.48,2.46,1.75
           ,2.19,2.78,1.45,3.46,4.16)
  p5C  = ggplot(df,aes(x=pos,y=mean_polymorphism,fill=color_group)) + geom_col(width=0.1,col="black") + theme_bw()+
...
}
{
  ...
  resolution=1
  jpeg(paste(path_figure,"p5AC.jpg",sep=""), width = 3600/resolution, height = 4000/resolution,res=350/resolution)
  print(p5AC)
  dev.off()
}
...

{
  ...
  p5B  = ggplot(df,aes(x=pos,y=mean_polymorphism,fill=color_group)) + geom_col(width=0.1,col="black") + theme_bw()+
...
df = data_5[data_5$filtering == "Homo_sapiens_rare_sv",]
{
  ...
  p5D  = ggplot(df,aes(x=pos,y=mean_polymorphism,fill=color_group)) + geom_col(width=0.1,col="black") + theme_bw()+
    geom_errorbar(aes(ymin=error_bar, ymax=error_bar_2),width=00.03,show.legend=FALSE)+ggtitle(" ")+ 
    geom_text(data=df,aes(x=pos-0.08,y=mean_polymorphism-0.002, family="serif",label=paste(round(Nb_introns_minor,3))),angle=90,vjust=0,size=6)+
    # geom_text(data=df,aes(x=pos+0.07,y=mean_polymorphism+0.004, family="serif",label=paste(round(percent_difference_vs_control,0))),angle=90,vjust=0,size=6)+
    # geom_text(data=df,aes(x=pos+0.07,y=mean_polymorphism+0.004, family="serif",label=significance),angle=90,vjust=0,size=6)+
...

{
  ...
  resolution=1
  jpeg(paste(path_figure,"p5BD.jpg",sep=""), width = 3600/resolution, height = 4000/resolution,res=350/resolution)
  print(p5BD)
  dev.off()
}
...
imgAC = load.image(paste(path_figure,"p5AC.jpg",sep=""))
imgBD = load.image(paste(path_figure,"p5BD.jpg",sep=""))
